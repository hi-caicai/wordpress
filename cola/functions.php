...
function par_pagenavi($range = 9){
	...
	if($max_page > 1)
	{
		...
		if(!$paged){$paged = 1; }
		if($paged != 1){echo "<a href='" . get_pagenum_link(1) . "' class='optional' title='跳转到首页'> 返回首页 </a>";}
		echo "<span class='prepage'>";
		previous_posts_link(' < ');
		echo "</span>";
		if($max_page > $range){
		...
		next_posts_link(' > ');
		echo "</span>";
		if($paged != $max_page){echo "<a href='" . get_pagenum_link($max_page) . "' class='optional' title='跳转到最后一页'> 最后一页 </a>";}
		echo "</div>";
	}
}
